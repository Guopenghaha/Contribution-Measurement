...
def get_data_for_federated_agents(source):#(前四个每个独占2类数据，后面一个均一分布,non-iid2)
    ...
    for i, sample in enumerate(Samples):
        if i == 0 or i == 1:
            ...
            num_samples_80 = int(num_samples * 0.8)
            all_samples[0].extend(sample[:num_samples_80])
            all_samples[4].extend(sample[num_samples_80:])
        elif i == 2 or i == 3:
            # 第2个客户端获取80%的2和3样本
            num_samples = len(sample)
            num_samples_80 = int(num_samples * 0.8)
            all_samples[1].extend(sample[:num_samples_80])
            all_samples[4].extend(sample[num_samples_80:])
        elif i == 4 or i == 5:
            # 第3个客户端获取80%的4和5样本
            num_samples = len(sample)
            num_samples_80 = int(num_samples * 0.8)
            all_samples[2].extend(sample[:num_samples_80])
            all_samples[4].extend(sample[num_samples_80:])
        elif i == 6 or i == 7:
            # 第4个客户端获取80%的6和7样本
            num_samples = len(sample)
            num_samples_80 = int(num_samples * 0.8)
            all_samples[3].extend(sample[:num_samples_80])
            all_samples[4].extend(sample[num_samples_80:])
        else:
            # 第5个客户端获取剩余的20%样本，并分配8和9样本
            num_samples = len(sample)
            num_samples_20 = int(num_samples * 0.2)
            all_samples[4].extend(sample[:num_samples_20])  # 剩余的20%样本
            all_samples[4].extend(random.sample(sample[num_samples_20:], num_samples_20))  # 随机选择20%的8和9样本

    min_samples = min(len(samples) for samples in all_samples)

    for no, no_agent in enumerate(all_samples):
        no_agent = random.sample(no_agent, min_samples)
    ...
    for agent_id, agent_data in enumerate(output_sequence):
        for data in agent_data:
            for label in data['y']:
                ...
                label_counts[agent_id, label_idx] += 1
...
if __name__ == "__main__":
    ...
    if DISTRIBUTION_TYPE == "SAME":
        federated_train_data_divide = get_data_for_federated_agents(mnist_train)
        ...
        for s in all_sets:
            group_shapley_value.append(
                train_with_gradient_and_valuation(s, gradient_weights, gradient_biases, gradient_lrs, DISTRIBUTION_TYPE,
                                                  iter_num, models_hository[iter_num-1],Dk))
