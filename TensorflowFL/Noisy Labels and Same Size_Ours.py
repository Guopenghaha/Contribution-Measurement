...
MIX_RATIO = 0.8
NUM_CLASSES = 10
NUM_DATA_PER_AGENT = 10840
NOISE_RATIOS = [0, 0.05, 0.10, 0.15, 0.20]
...
def get_data_for_federated_agents1(source, num):#noise
    output_sequence = []
...
def get_data_for_federated_agents(source, num):
    ...
    for i in range(0, len(all_samples), BATCH_SIZE):
        ...
        output_sequence.append({
            ...
            'y': np.array([source[1][i] for i in batch_samples], dtype=np.int32),
            'noise': np.zeros((len(batch_samples), 28 * 28), dtype=np.float32)  # 添加噪声数据的占位符
        })

    ratio = num * 0.05
    sum_agent = int(len(all_samples))

    for i in range(sum_agent):
        noiseHere = ratio * np.random.randn(28 * 28)
        batch_index = int(i / BATCH_SIZE)
        sample_index = i % BATCH_SIZE
        output_sequence[batch_index]['x'][sample_index] = checkRange(
            np.add(output_sequence[batch_index]['x'][sample_index], noiseHere)
        )
        output_sequence[batch_index]['noise'][sample_index] = noiseHere  # 记录噪声数据

    return output_sequence

def count_agent_label_counts(num_agents, num_data_per_agent, num_classes, noise_ratios):
    # 初始化噪声分布
    noise_distribution = np.zeros((num_agents, num_classes), dtype=int)

    for agent_id in range(num_agents):
        ratio = noise_ratios[agent_id]
        total_noisy_data = int(num_data_per_agent * ratio)
        noisy_data_per_class = np.random.multinomial(total_noisy_data, [1 / num_classes] * num_classes)
        noise_distribution[agent_id, :] = noisy_data_per_class
...
def train_with_gradient_and_valuation(agent_list, grad, bi, lr, distr_type, datanum, iter_n, g_m,Dk):
    ...
    for i in agent_list:
        ...
        # # # agents_wn[i] = datanum[i] / data_sum
        # agents_wn[i] = 1 / len(agent_list)
    for i in range(iter_n - 1, iter_n):
        ...
        for j in agent_list:
            #ddk = Dk[j]
            #agents_wn= agents_w -a*Dk+b # 计算pk
            # if np.sum(tmp > 0) > 0:  # ensure not all elements are smaller than 0
            #     agents_w  = np.copy(tmp)
            #     agents_w [agents_w  < 0.0] = 0.0
            # total_normalizer = sum([agents_w [j] for j in range(len(agent_list))])
            # agents_w  = [agents_w [j] / total_normalizer for j in range(len(agent_list))]
...
if __name__ == "__main__":
    ...
    label_counts = count_agent_label_counts(NUM_AGENT, NUM_DATA_PER_AGENT , NUM_CLASSES, NOISE_RATIOS)
    traindata_cls_counts = label_counts
    # print(traindata_cls_counts)
    modified_matrix = np.vstack((np.full((1, 10), 1e-8), traindata_cls_counts[1:]))
    print(modified_matrix)
    # exit()
    #计算差异
    party_list_rounds = [i for i in range(NUM_AGENT)]  # 所有可能的参与方
    distribution_difference = get_distribution_difference(modified_matrix, participation_clients=party_list_rounds,
                                                          metric='kl')
    # 计算Dk
