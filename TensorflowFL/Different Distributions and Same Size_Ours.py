...
def get_data_for_federated_agents(source):#no-iid1
    output_sequence = [[] for _ in range(NUM_AGENT)]

    Samples = []
    for digit in range(0, 10):
        # labels = np.argmax(source[1], axis=1)
    ...
    for i, sample in enumerate(Samples):
        ...
        for j in range(NUM_AGENT):
            if j == 4:  # 第 4 个客户端独占数字 8 和 9 的样本
                if i == 8 or i == 9:  # 数字 8 和 9 的样本
                    all_samples[j].extend(sample)
            elif i == j * 2 or i == j * 2 + 1:
    ...
    for iter_num in range(1, len(gradient_weights[0])+1):
        ...
        for s in all_sets:
            group_shapley_value.append(
                ...
                #                                   data_num, iter_num, models_hository[iter_num-1],normalized_acc))
                train_with_gradient_and_valuation(s, gradient_weights, gradient_biases, gradient_lrs, DISTRIBUTION_TYPE,
